...
| GET    | `/api/ui/enrich/stream?ioc=<v>[&defang&no_cache]` | SSE stream of per-source progress for one IOC. Events: `meta` (`{ioc, type, pending}`) once at start; `module` (`{name, entry}`) as each source resolves via `asyncio.as_completed`; `final` (the same UI-shaped payload `/api/ui/enrich` returns). Lets the dashboard render the fastest source immediately instead of waiting on the slowest. Same auth + `?defang` semantics as the blocking endpoint. |
