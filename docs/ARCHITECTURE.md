...
| GET    | `/api/ui/enrich/stream?ioc=<v>[&defang&no_cache]` | SSE stream of per-source progress for one IOC. Events: `meta` (`{ioc, type, pending}`) once at start; `module` (`{name, entry}`) as each source resolves via `asyncio.as_completed`; `skipped` (`{name}`) when a source returns `None` (no data / API key missing); `final` (the same UI-shaped payload `/api/ui/enrich` returns). `module` + `skipped` counts sum to `pending` so the dashboard's landed/total counter doesn't stall on soft-fail sources. Same auth + `?defang` semantics as the blocking endpoint. |
