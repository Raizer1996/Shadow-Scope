...
- [x] **Per-source rate-limit handler** — new `core/ratelimit.py` token-bucket registry (per-source defaults for VT/AbuseIPDB/Shodan/OTX/URLscan/Pulsedive/GreyNoise/AbstractAPI/Censys/IPQS, override via `RATELIMIT_<KEY>=cap/period` env, disable via `RATELIMIT_<KEY>_DISABLE=1`). New `core/http.py` wraps `requests` with bucket gating + single 429 retry honouring `Retry-After`. VT module migrated as the reference conversion.
